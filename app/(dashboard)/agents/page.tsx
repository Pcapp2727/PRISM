...
import { countUntrackedDecisions } from '@/lib/bridge-data';
...
export default async function AgentsPage() {
  const [fleet, untrackedCount] = await Promise.all([
    getAgentFleet(),
    countUntrackedDecisions().catch(() => 0),
  ]);
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-10">
      ...
      {untrackedCount > 0 && (
        <Link
          href="/bridge"
          className="block mb-6 border border-[#5B9DF5]/20 bg-[#5B9DF5]/[0.03] rounded-lg p-4 hover:bg-[#5B9DF5]/[0.06] transition-colors"
        >
          <div className="flex items-center justify-between gap-4">
            <div>
              <div className="text-sm text-[#5B9DF5]">
                {untrackedCount} PRISM {untrackedCount === 1 ? 'analysis' : 'analyses'} ready to track
              </div>
              <div className="text-xs text-white/45 mt-0.5">
                Promote them into the agent fleet to start building calibration data.
              </div>
            </div>
            <span className="text-[#5B9DF5] shrink-0">→</span>
          </div>
        </Link>
      )}
  ...
  return (
    <div className="border border-white/[0.06] rounded-lg p-10 text-center">
      ...
      <p className="text-sm text-white/40 max-w-md mx-auto">
        Register an agent in your MCP server config, then have it call <code className="text-[#5B9DF5]">should_gate</code>.
      </p>
    </div>
  );
}
